...
#[test] fn anon_func_no_args() {
    ok("() { echo anon; }", "anon\n");
}
#[test] fn anon_func_with_args() {
...
#[test] fn shift_drops_first_param() {
    ok("set -- a b c; shift; echo $1 $2", "b c\n");
}
#[test] fn shift_n_drops_n_params() {
    ok("set -- a b c d; shift 2; echo $1 $2", "c d\n");
}
#[test] fn set_double_dash_resets_positionals() {
...
#[test] fn substring_offset_length() {
    ok("v=hello; echo ${v:1:3}", "ell\n");
}
#[test] fn substring_offset_only() {
    ok("v=hello; echo ${v:2}", "llo\n");
}
#[test] fn string_length_in_cond() {
...
#[test] fn replace_first() {
    ok(r#"v=hello; echo ${v/l/L}"#, "heLlo\n");
}
#[test] fn replace_all() {
    ok(r#"v=hello; echo ${v//l/L}"#, "heLLo\n");
}
#[test] fn replace_anchor_start() {
    ok(r#"v=hello; echo ${v/#hel/HEL}"#, "HELlo\n");
}
#[test] fn replace_anchor_end() {
...
#[test] fn arith_bit_and_ext() { ok("echo $((5 & 3))", "1\n"); }
#[test] fn arith_bit_or_ext() { ok("echo $((5 | 3))", "7\n"); }
#[test] fn arith_bit_xor_ext() { ok("echo $((5 ^ 3))", "6\n"); }
#[test] fn arith_left_shift_ext() { ok("echo $((1 << 4))", "16\n"); }
#[test] fn arith_right_shift_ext() { ok("echo $((16 >> 2))", "4\n"); }
#[test] fn arith_modulo_ext() { ok("echo $((17 % 5))", "2\n"); }
#[test] fn arith_negate_ext() { ok("echo $((-5))", "-5\n"); }
#[test] fn arith_logical_and_ext() { ok("echo $((1 && 1))", "1\n"); }
#[test] fn arith_logical_or_ext() { ok("echo $((0 || 1))", "1\n"); }
#[test] fn arith_compound_assign_ext() { ok("x=5; (( x += 3 )); echo $x", "8\n"); }
#[test] fn arith_pow_ext() { ok("echo $((2 ** 10))", "1024\n"); }

// ─────────────────────────────────────────────────────────────────────────────
// Glob patterns
// ─────────────────────────────────────────────────────────────────────────────

#[test] fn glob_question_mark() {
...
#[test] fn array_negative_index() {
    ok("a=(red green blue); echo ${a[-1]}", "blue\n");
}
#[test] fn array_first_via_index_one() {
...
#[test] fn dollar_question_chains() {
    ok("true; echo $?; false; echo $?", "0\n1\n");
}
#[test] fn dollar_zero_in_script() {
    let (status, out) = run("echo $0");
    assert_eq!(status, 0);
    // $0 is the shell name; non-empty
    assert!(!out.trim().is_empty());
}
#[test] fn dollar_pipestatus() {
...
#[test] fn case_glob_pattern_ext() {
    ok("case foo.txt in *.txt) echo text ;; *) echo other ;; esac", "text\n");
}
#[test] fn case_alternation() {
    ok("case red in red|blue) echo primary ;; *) echo other ;; esac", "primary\n");
}
#[test] fn case_char_class() {
    ok(
...
#[test] fn heredoc_with_var_expansion() {
    ok("v=world; cat <<EOF\nhi $v\nEOF", "hi world\n");
}
#[test] fn heredoc_quoted_terminator_no_expansion() {
    ok("cat <<'EOF'\n$NOT_EXPANDED\nEOF", "$NOT_EXPANDED\n");
}
#[test] fn heredoc_dash_strips_tabs() {
    ok("cat <<-EOF\n\tline\n\tEOF", "line\n");
}
...
#[test] fn function_multi_stmt() {
    ok(
        "f() { local x=1; local y=2; echo $((x+y)); }; f",
        "3\n",
    );
}
#[test] fn function_call_chain() {
    ok(
...
#[test] fn trap_unset_with_dash() {
    ok(
        "trap 'echo nope' EXIT; trap - EXIT; echo done",
        "done\n",
    );
}
...
#[test] fn pipeline_with_function() {
    ok_serial(
        "f() { echo from-f; }; f | tr a-z A-Z",
        "FROM-F\n",
    );
}
#[test] fn pipeline_into_read() {
    ok_serial(
        "echo hello | { read line; echo got=$line; }",
        "got=hello\n",
    );
}
#[test] fn pipeline_with_subshell() {
    ok_serial(
...
#[test] fn short_circuit_left_succeeds() {
    ok("true && echo y || echo n", "y\n");
}
#[test] fn short_circuit_left_fails() {
    ok("false && echo y || echo n", "n\n");
}
#[test] fn three_way_chain_first_fails() {
...
#[test] fn local_does_not_leak() {
    ok(
        "x=outer; f() { local x=inner; }; f; echo $x",
        "outer\n",
    );
}
#[test] fn local_visible_in_nested_call() {
    ok(
...
#[test] fn for_implicit_positional() {
    ok(
...
#[test] fn printf_multi_args_loops_format() {
    ok("printf '%s\\n' a b c", "a\nb\nc\n");
}
#[test] fn printf_d_format() {
    ok("printf '%d\\n' 42", "42\n");
}
#[test] fn echo_e_interprets_escapes() {
...
#[test] fn subshell_assignment_no_leak() {
    ok("x=outer; (x=inner; true); echo $x", "outer\n");
}
#[test] fn subshell_cd_no_leak() {
    let saved = std::env::current_dir().unwrap();
    let saved_str = saved.to_string_lossy().into_owned();
    let (status, out) = run(&format!(
        "(cd /tmp; echo $PWD) > /dev/null; pwd"
    ));
    assert_eq!(status, 0);
...
#[test] fn comment_at_eol() { ok("echo a # tail comment", "a\n"); }
#[test] fn comment_full_line() { ok("# only comment\necho ok", "ok\n"); }
#[test] fn hash_in_string_not_comment() {
    ok(r#"echo "a#b""#, "a#b\n");
}
...
#[test] fn many_newlines_between_commands() {
    ok("echo a\n\n\n\n\necho b", "a\nb\n");
}
#[test] fn semis_between_commands() {
...
#[test] fn recursive_function_with_cmd_subst() {
    ok(
...
#[test] fn function_with_for_and_break() {
    ok(
        "f() { for i in 1 2 3 4; do (( i == 3 )) && break; echo $i; done; }; f",
        "1\n2\n",
    );
}
#[test] fn function_with_continue() {
    ok(
...
#[test] fn redirect_stdout_and_stderr_separately() {
    ...
    assert_eq!(status, 0);
    assert_eq!(std::fs::read_to_string(&p_out).unwrap_or_default(), "to-out\n");
    assert_eq!(std::fs::read_to_string(&p_err).unwrap_or_default(), "to-err\n");
    let _ = std::fs::remove_file(&p_out);
    let _ = std::fs::remove_file(&p_err);
}
...
#[test] fn process_sub_in_diff_two() {
...
#[test] fn param_alt_set() { ok("v=value; echo ${v:+ALT}", "ALT\n"); }
#[test] fn param_alt_unset() { ok("echo ${unset:+ALT}", "\n"); }
#[test] fn param_assign_default_ext() {
    ok("echo ${unset:=fallback}; echo $unset", "fallback\nfallback\n");
}
#[test] fn param_strip_long_prefix_ext() {
    ok("p=a/b/c/d; echo ${p##*/}", "d\n");
}
#[test] fn param_strip_long_suffix_ext() {
    ok("p=a.tar.gz; echo ${p%%.*}", "a\n");
}
#[test] fn substring_negative_offset() {
    ok("v=abcdef; echo ${v: -3}", "def\n");
}
#[test] fn double_brace_indirect() {
    // ${${...}} pattern — basic form
...
#[test] fn cond_v_set() { ok("v=anything; [[ -v v ]] && echo y", "y\n"); }
#[test] fn cond_v_unset() {
    ok_status("[[ -v unset_var_here ]] && echo y", "", 1);
}
#[test] fn cond_pat_glob() { ok("[[ abc == a* ]] && echo y", "y\n"); }
#[test] fn cond_double_neg() {
    ok("[[ ! ! a == a ]] && echo y", "y\n");
}
#[test] fn cond_or_chain() {
    ok(r#"[[ x == y || x == x ]] && echo y"#, "y\n");
}
#[test] fn cond_string_le() { ok("[[ a < b ]] && echo y", "y\n"); }

// ─────────────────────────────────────────────────────────────────────────────
// Set / unset options
// ─────────────────────────────────────────────────────────────────────────────

#[test] fn set_exit_on_error_true_path() {
    // set -e: doesn't trip on conditional expressions
    ok("set -e; true && echo y", "y\n");
}
#[test] fn set_unset_var_warns() {
    // Without `set -u`, unset vars expand to empty silently.
    ok("echo before; echo \"[$undefined_var]\"; echo after", "before\n[]\nafter\n");
}
...
#[test] fn read_multi_into_two_vars() {
    ok("echo a b | { read x y; echo $x-$y; }", "a-b\n");
}
#[test] fn read_with_default_var_REPLY() {
...
#[test] fn dollar_underscore_after_cmd() {
...
#[test] fn case_fallthrough_semi_amp() {
    ok(
...
#[test] fn pipe_amp_includes_stderr() {
    ok_serial(
...
#[test] fn arith_compare_eq_zero_status() {
    ok_status("(( 5 == 5 )); echo $?", "0\n", 0);
}
#[test] fn arith_compare_neq_one_status() {
    ok_status("(( 5 == 6 )); echo $?", "1\n", 0);
}
#[test] fn arith_ternary_eval() {
    ok("echo $((5 > 3 ? 100 : 200))", "100\n");
}
#[test] fn arith_pre_inc_ext() {
    ok("i=5; echo $((++i)); echo $i", "6\n6\n");
}
#[test] fn arith_post_inc_ext() {
...
#[test] fn brace_alt_with_prefix_suffix() {
    ok("echo pre{a,b,c}post", "preapost prebpost precpost\n");
}
#[test] fn brace_range_step() {
    ok("echo {1..5..2}", "1 3 5\n");
}
#[test] fn brace_letter_range() {
    ok("echo {a..d}", "a b c d\n");
}
#[test] fn brace_nested_alt() {
...
#[test] fn local_array_in_function() {
    ok(
        "f() { local a=(x y z); echo ${a[2]}; }; f",
        "y\n",
    );
}
...
#[test] fn array_length_with_at() {
    ok("a=(x y z); echo ${#a[@]}", "3\n");
}
#[test] fn array_iterate_indexed() {
    ok(
...
#[test] fn function_with_empty_body() {
    ok("f() { :; }; f; echo $?", "0\n");
}
#[test] fn function_returns_status() {
    ok_status("f() { return 7; }; f; echo $?", "7\n", 0);
}
...
#[test] fn comment_after_command_continuation() {
    ok("echo a # comment\necho b", "a\nb\n");
}
...
#[test] fn pipe_empty_command_status() {
...
#[test] fn export_visible_to_child() {
    ok_serial(
...
#[test] fn string_eq_with_dquoted_pattern() {
    ok(r#"x=hi; [[ "$x" = "hi" ]] && echo y"#, "y\n");
}
#[test] fn string_pattern_no_match() {
...
#[test] fn arith_var_assign_compound() {
    ok("x=10; (( x *= 3 )); echo $x", "30\n");
}
#[test] fn arith_two_vars() {
...
#[test] fn exit_with_status_propagates() {
    ok_status("(exit 42); echo $?", "42\n", 0);
}
#[test] fn return_outside_function_is_error_or_exit() {
...
#[test] fn glob_qualifier_dot_files_only() {
...
#[test] fn glob_qualifier_slash_dirs_only() {
...
#[test] fn glob_qualifier_N_nullglob() {
...
#[test] fn pipeline_chained_functions() {
    ok_serial(
        "f() { echo hello; }; g() { tr a-z A-Z; }; f | g",
        "HELLO\n",
    );
}
#[test] fn pipeline_with_arith_count() {
    ok_serial(
...
#[test] fn getopts_no_args_done_immediately() {
    ok(
        r#"set --
while getopts "ab" opt; do
  case $opt in
...
#[test] fn arith_compare_in_cond() {
    ok("a=5; b=3; [[ $a -gt $b ]] && echo y", "y\n");
}
#[test] fn arith_compare_double_paren() {
...
#[test] fn path_basename_via_param() {
    ok("p=a/b/c.txt; echo ${p##*/}", "c.txt\n");
}
#[test] fn path_dirname_via_param() {
    ok("p=a/b/c.txt; echo ${p%/*}", "a/b\n");
}
#[test] fn path_extension_via_param() {
    ok("p=a/b/c.txt; echo ${p##*.}", "txt\n");
}
#[test] fn path_basename_no_ext() {
...
#[test] fn heredoc_in_brace_group() {
    ok(
        "{ cat <<EOF
inside
EOF
...
#[test] fn function_assigned_var_visible_outside() {
...
#[test] fn local_array_does_not_leak() {
    ok(
...
#[test] fn three_cmds_on_one_line() {
    ok("echo one; echo two; echo three", "one\ntwo\nthree\n");
}
#[test] fn cmds_with_var_in_between() {
...
#[test] fn group_with_or_chain() {
    ok("{ false; } || echo y", "y\n");
}
#[test] fn group_with_and_chain() {
    ok("{ true; } && echo y", "y\n");
}
...
#[test] fn glob_recursive_globstar() {
...
#[test] fn read_with_r_preserves_backslash() {
    ok_serial(
...
#[test] fn assignment_then_external_via_func() {
    ok(
        "f() { local v=$1; echo got=$v; }; f hello",
        "got=hello\n",
    );
}
...
#[test] fn funcname_inside_function() {
...
#[test] fn set_o_no_args_no_error() {
    let (status, _) = run("set -o");
    assert_eq!(status, 0);
}
#[test] fn set_dash_e_then_unset() {
    ok(
...
#[test] fn array_subscript_assign() {
    ok("a=(x y z); a[2]=NEW; echo ${a[2]}", "NEW\n");
}
#[test] fn assoc_subscript_long_key() {
    ok(
...
#[test] fn heredoc_with_arith_sub() {
    ok(
        "cat <<EOF
result: $((3 + 4))
EOF",
...
#[test] fn until_with_compound_cond() {
    ok(
...
#[test] fn function_recursion_to_depth() {
    ok(
...
#[test] fn proc_sub_with_redirect() {
    ok_serial(
        "/bin/cat <(echo first) <(echo second)",
        "first\nsecond\n",
    );
}
...
#[test] fn pipeline_negate_status() {
...
#[test] fn eval_multi_stmt() {
    ok("eval 'echo a; echo b; echo c'", "a\nb\nc\n");
}
#[test] fn eval_with_var_capture() {
...
#[test] fn concat_arith_in_string() {
...
#[test] fn empty_array_assignment() {
    ok("a=(); echo ${#a[@]}", "0\n");
}
...
#[test] fn return_in_nested_function() {
    ok(
