...
mod noclobber_on {
    ...
    fn no_clobber_blocks_overwrite() {
        ...
        assert_parity_in(d.path(), "setopt no_clobber; echo new > exists.txt 2>/dev/null; echo exit=$?; cat exists.txt");
    }
    ...
    fn no_clobber_allows_new_file() {
        let d = tdir();
        assert_parity_in(d.path(), "setopt no_clobber; echo new > brandnew.txt; cat brandnew.txt");
    }
    ...
    fn pipe_bar_force_clobber_works() {
        let d = tdir();
        std::fs::write(d.path().join("exists.txt"), "old\n").unwrap();
        assert_parity_in(d.path(), "setopt no_clobber; echo new >| exists.txt; cat exists.txt");
    }
    ...
    fn noclobber_append_existing_works() {
        let d = tdir();
        std::fs::write(d.path().join("exists.txt"), "old\n").unwrap();
        assert_parity_in(d.path(), "setopt no_clobber; echo more >> exists.txt; cat exists.txt");
    }
}
...
mod noclobber_off {
    ...
    fn unset_no_clobber_restores_overwrite() {
        let d = tdir();
        std::fs::write(d.path().join("out.txt"), "old\n").unwrap();
        assert_parity_in(d.path(), "setopt no_clobber; unsetopt no_clobber; echo new > out.txt; cat out.txt");
    }
}
...
mod no_clobber_alias {
    ...
    fn noclobber_no_underscore_works() {
        let d = tdir();
        std::fs::write(d.path().join("exists.txt"), "old\n").unwrap();
        assert_parity_in(d.path(), "setopt NOCLOBBER; echo new > exists.txt 2>/dev/null; echo exit=$?");
    }
}
