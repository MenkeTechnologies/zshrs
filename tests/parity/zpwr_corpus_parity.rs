...
mod zpwr_common {
    ...
    fn q_strip_then_chain() {
        assert_parity(
            r###"firstArg="\"/tmp/foo/bar.txt\""; v="${(Q)firstArg}"; print -r -- "${v:h} :: ${v:t} :: ${v:e} :: ${v:r}""###,
        );
    }
    ...
    fn az_at_subscript_strip() {
        assert_parity(
            r###"set -- "\"/a/b/c.txt\"" two three; firstArg="${${(Az)@}[1]//\"/}"; print -r -- "$firstArg""###,
        );
    }
    ...
    fn at_offset_last_positional() {
        assert_parity(
            r###"set -- a b https://github.com/u/repo.git; last_arg=${@: -1}; git_name=${last_arg##*/}; dir_name=${git_name%.*}; print -r -- "$last_arg | $git_name | $dir_name""###,
        );
    }
    ...
    fn neg_offset_and_strip_last() {
        assert_parity(
            r###"noproto="example.com."; print -r -- "last=[${noproto: -1}] strip=[${noproto:0:-1}]""###,
        );
    }
    ...
    fn gsub_var_pattern_redact() {
        assert_parity(
            r###"GITHUB_TOKEN="abc123secret"; cmd="curl -H token:abc123secret https://api"; print -r -- "${cmd//$GITHUB_TOKEN/***}""###,
        );
    }
    ...
    fn base_output_zero_pad() {
        assert_parity(
            r###"n=255; print -r -- "${(l:4::0:)$(([##16]n))}"; print -r -- "$(([##2]10))""###,
        );
    }
    ...
    fn type_flag_three_kinds() {
        assert_parity(
            r###"typeset -a arr=(1 2); typeset -A h=(k v); s=str; print -r -- "${(t)arr} | ${(t)h} | ${(t)s}""###,
        );
    }
    ...
    fn backref_gsub_reorder() {
        assert_parity(
            r###"setopt extendedglob; v="key=val"; print -r -- "${v//(#b)(*)=(*)/${match[2]}:${match[1]}}""###,
        );
    }
    ...
    fn match_ref_wrap_digits() {
        assert_parity(
            r###"setopt extendedglob; v="abc123def"; print -r -- "${v//(#m)[0-9]##/<$MATCH>}""###,
        );
    }
    ...
    fn count_assoc_keys_glob() {
        assert_parity(
            r###"typeset -A h=(apple 1 apricot 2 banana 3); print -r -- "count=${#h[(I)a*]}""###,
        );
    }
    ...
    fn sort_quoted_vs_unquoted() {
        assert_parity(
            r###"a=(c a b); print -r -- "joined=[${(o)a}]"; print -r -- "split=${(o)a}""###,
        );
    }
    ...
    fn buffer_arith_slice() {
        assert_parity(
            r###"BUFFER="git commit -m msg"; CURSOR=10; print -r -- "L=[${BUFFER[1,CURSOR]}] R=[${BUFFER[CURSOR+1,-1]}]""###,
        );
    }
    ...
    fn glob_plain_dir_basename() {
        assert_parity(
            r###"d=$(mktemp -d); : > $d/a.txt; : > $d/b.log; mkdir $d/sub; files=( $d/*(.N:t) ); dirs=( $d/*(/N:t) ); print -r -- "files=${(o)files}"; print -r -- "dirs=${(o)dirs}"; rm -rf $d"###,
        );
    }
...
print ${+parameters[MYEXPORTVAR]}"###,
    ...
    fn regex_capture_buffer_rewrite() {
        assert_parity(
            r###"local LBUFFER="echo a; set -x; echo b"
if [[ $LBUFFER =~ "^(.*)((set)[[:space:]]+(-x)[[:space:]]*;+[[:space:]]*)+(.*)$" ]]; then
  print "1=[$match[1]] 5=[$match[5]]"
    ...
    fn functions_body_capture() {
        assert_parity(
            r###"realfn(){ print hello; print world; }
...
for cmd in "touch" "touchtest" "touch foo"; do
  if [[ ${cmd} == touch([^[:alnum:]_]*|) ]]; then
    ...
    fn double_nested_field_strip() {
        assert_parity(
            r###"local -a entries=( "1000 12.5 3.2 1 my_func" )
...
print "time: ${${entries[1]#* }%% *}""###,
    ...
    fn count_matching_keys_nested() {
        assert_parity(
            r###"local -A comps=( _git 1 _zpwr_foo 1 _ls 1 _zpwr_bar 1 )
...
print $(( 10#$oct ))"###,
    ...
    fn glob_type_qualifiers() {
        assert_parity(
            r###"setopt extendedglob nullglob
local d=$(mktemp -d)
...
for k in ${(ok)c}; do print "$k=$c[$k]"; done"###,
    ...
    fn array_slice_join() {
        assert_parity(
            r###"local -a a=( one two three four five )
print "${a[2,4]}"
print "${a[-1]}"
print "${(j:,:)a[2,-1]}""###,
        );
...
print -l $kept"###,
    ...
    fn index_last_match_membership() {
        assert_parity(
            r###"local -a w=( ping curl git nmap )
...
print "membership-test: $(( ${w[(I)nmap]} > 0 ))""###,
    ...
    fn unique_first_seen() {
        assert_parity(
            r###"local -a a=( foo bar foo baz bar foo )
print -l ${(u)a}"###,
        );
    }
...
print -l $out"###,
    ...
    fn at_M_matched_keep() {
        assert_parity(
            r###"local -a items=( apple_pie banana cherry_cake apricot )
...
print -l $matched"###,
    ...
    fn indirect_then_substr() {
        assert_parity(
            r###"local LONGVAR="0123456789abcdef"
local name=LONGVAR
print "${(P)name:0:5}""###,
        );
    }
...
print "rest=$*""###,
    ...
    fn zstyle_roundtrip() {
        assert_parity(
            r###"zstyle ":completion:*:foo" group-name "mygroup"
local val
...
mod zpwr_env {
    ...
    fn strip_then_semicolon_split() {
        assert_parity(
            r###"v="forgit::add;forgit::warn=desc"; cmd=${v%%=*}; for exp in ${(s%;%)cmd}; do print -- "$exp"; done"###,
        );
    }
    ...
    fn tied_typeset_T() {
        assert_parity(
            r###"typeset -T FOO foo ":"; foo=(a b c); print -r -- "$FOO"; print -r -- "${foo[2]}""###,
        );
    }
    ...
    fn tied_append_rebuild() {
        assert_parity(
            r###"typeset -T MYPATH mypath ":"; mypath=(/a /b /c); print -r -- "$MYPATH"; mypath+=(/d); print -r -- "$MYPATH""###,
        );
    }
    ...
    fn named_directory() {
        assert_parity(
            r###"hash -d zp=/tmp/zpwr/repo; print -r -- ${nameddirs[zp]}; print -r -- ~zp/scripts"###,
        );
    }
    ...
    fn array_diff_intersect() {
        assert_parity(
            r###"a=(rm mv cp ln chmod); b=(cp ln); d=(${a:|b}); i=(${a:*b}); print -r -- "diff=$d"; print -r -- "int=$i""###,
        );
    }
    ...
    fn ps_preserve_empty_tab() {
        assert_parity(
            r###"data=$'a\tb\tc'; arr=(${(ps:\t:)data}); print "n=$#arr"; print -- "${arr[2]}""###,
        );
    }
    ...
    fn typeset_aU_dedup_append() {
        assert_parity(
            r###"typeset -aU u=(rm mv rm cp mv); print -r -- "$u"; u+=(rm); print "after add n=$#u""###,
        );
    }
    ...
    fn global_aliases_hash() {
        assert_parity(
            r###"alias -g G="| grep" PIPE="2>&1"; for k in ${(ko)galiases}; do print -- "$k -> ${galiases[$k]}"; done"###,
        );
    }
    ...
    fn suffix_aliases_hash() {
        assert_parity(
            r###"alias -s txt=cat pdf=open; for k in ${(ko)saliases}; do print -- "$k=${saliases[$k]}"; done"###,
        );
    }
    ...
    fn sorted_keys_and_values() {
        assert_parity(
            r###"typeset -gA H=(z 26 a 1 m 13); print -r -- ${(ko)H}; print -r -- ${(vo)H}"###,
        );
    }
    ...
    fn parameters_type_strings() {
        assert_parity(
            r###"typeset -gA AA=(x 1); typeset -ax LL=(a b); typeset -gix N=5; print "AA=${parameters[AA]}"; print "LL=${parameters[LL]}"; print "N=${parameters[N]}""###,
        );
    }
    ...
    fn hash_iter_value_split() {
        assert_parity(
            r###"typeset -gA H; H[a]="f1;f2"; H[b]="g1"; for k in ${(ko)H}; do for e in ${(s%;%)H[$k]}; do print "$k:$e"; done; done"###,
        );
    }
    ...
    fn backref_in_test() {
        assert_parity(
            r###"setopt extendedglob; s="=37;45"; if [[ $s == (#b)=(*) ]]; then print "cap=$match[1]"; fi"###,
        );
    }
}
...
mod zpwr_lib {
    ...
    fn index_exact_membership() {
        assert_parity(
            r###"typeset -a bl=(idna six certifi); pkg=six; if (( bl[(Ie)$pkg] )); then print "found at ${bl[(Ie)$pkg]}"; fi; pkg=nope; print "missing=${bl[(Ie)$pkg]}""###,
        );
    }
    ...
    fn tP_array_vs_scalar() {
        assert_parity(
            r###"typeset -ax arr=(1 2); typeset -gx str=hi; n=arr; print -rl -- ${(tP)n}; n=str; print -rl -- ${(tP)n}"###,
        );
    }
    ...
    fn indirect_array_subscript() {
        assert_parity(
            r###"typeset -a colors=(red green blue); name=colors; print -r -- ${(P)name}; print -r -- ${${(P)name}[2]}"###,
        );
    }
    ...
    fn keep_matched_portion() {
        assert_parity(
            r###"s="foo=bar=baz"; print -r -- "${(M)s#*=}"; print -r -- "${(M)s%%[0-9]*}""###,
        );
    }
    ...
    fn anchored_substitutions() {
        assert_parity(
            r###"s="a.b.c"; print -- "${s//./_}"; print -- "${s/#a/X}"; print -- "${s/%c/Z}""###,
        );
    }
...
mod zpwr_scripts {
    ...
    fn lastwrite_assoc_sorted_keys() {
        assert_parity(
            r###"data=("param foo" "alias bar" "func baz")
...
for n in $sorted; do print -r -- "${key_by_name[$n]}"; done"###,
    ...
    fn sort_flag_variants() {
        assert_parity(
            r###"arr=(zebra apple Mango banana)
print -r -- ${(on)arr}
print -r -- ${(oin)arr}
print -r -- ${(On)arr}"###,
        );
...
print -r -- ${p:e}"###,
    ...
    fn ie_blacklist_membership() {
        assert_parity(
            r###"BLACKLIST=(shutdown halt reboot)
...
print -r -- ${(on)${(k)info}}"###,
    ...
    fn match_count_two_forms() {
        assert_parity(
            r###"a=(apple apricot banana avocado)
print -r -- "${#a[(I)a*]}"
print -r -- "${(M)#a:#a*}""###,
        );
    }
...
print -r -- "${${${${CONDA_PROMPT_MODIFIER#\(}% }%\)}:-${CONDA_PREFIX:t}}""###,
    ...
    fn substr_conditional_ellipsis() {
        assert_parity(
            r###"P9K_CONTENT="this is a very long string over twenty four chars"
...
print -r -- "${short:0:24}${${short:24}:+…}""###,
    ...
    fn negmatch_in_plus() {
        assert_parity(
            r###"P9K_CONTENT="3.11.0"; P9K_PYENV_PYTHON_VERSION="3.11.0"
...
print -r -- "${P9K_CONTENT}${${P9K_PYENV_PYTHON_VERSION:#$P9K_CONTENT}:+ $P9K_PYENV_PYTHON_VERSION}""###,
    ...
    fn synthesize_then_filter() {
        assert_parity(
            r###"P9K_KUBECONTEXT_NAMESPACE=default
...
print"###,
    ...
    fn unset_m_pattern() {
        assert_parity(
            r###"typeset -g FOO_A=1 FOO_B=2 BAR=3
unset -m "FOO_*"
