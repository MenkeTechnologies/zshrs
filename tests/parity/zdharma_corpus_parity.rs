...
parse "weird://nope"; print "rc=$?""###,
    ...
    fn zn_word_offsets() {
        assert_parity(
            r###"setopt extendedglob typesetsilent noshortloops
...
mod zui_zinit {
    ...
    fn page_offset() {
        assert_parity(
            r###"integer page_height=10 current_idx=37 last=100
...
print "from=$from end=$end""###,
    ...
    fn scroll_clamp() {
        assert_parity(
            r###"integer initial=3 height=10 size=5
integer start end
...
for (( i=1; i<=ssize; i+=2 )); do
  ...
  print -r -- "$k=$v"
    ...
    fn marker_fold_dedup() {
        assert_parity(
            r###"arr=( foo_-_bar baz_-_qux foo_-_bar Script_Body_ )
arr=( "${arr[@]//_-_//}" )
...
decode "db_45_name_46_tbl""###,
    ...
    fn history_dedup() {
        assert_parity(
            r###"hist=( "git commit -m foo" "ls -la /tmp" "git commit -m bar" "ls -la /tmp" )
typeset -A seen
local -a cmds
...
print -rl -- "${cmds[@]}""###,
    ...
    fn freq_sort() {
        assert_parity(
            r###"words=( apple pear apple banana pear apple )
typeset -A freq
