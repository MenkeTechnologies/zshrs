...
mod zmod_math {
    ...
    #[test]
    fn sqrt_cbrt_frac() {
        assert_parity(r###"zmodload zsh/mathfunc; printf "%.6f %.6f\n" $(( sqrt(2) )) $(( cbrt(2) ))"###);
    }
    ...
    #[test]
    fn logs() {
        assert_parity(r###"zmodload zsh/mathfunc; echo $(( log(1) )) $(( log10(1000) )) $(( log2(8) ))"###);
    }

    /// sin/cos/tan at 0.
    #[test]
    fn trig_zero() {
        assert_parity(r###"zmodload zsh/mathfunc; echo $(( sin(0) )) $(( cos(0) )) $(( tan(0) ))"###);
    }

    /// asin/acos/atan.
    #[test]
    fn inverse_trig() {
        assert_parity(r###"zmodload zsh/mathfunc; printf "%.6f %.6f %.6f\n" $(( asin(1) )) $(( acos(0) )) $(( atan(1) ))"###);
    }
    ...
    #[test]
    fn hyperbolic_zero() {
        assert_parity(r###"zmodload zsh/mathfunc; echo $(( sinh(0) )) $(( cosh(0) )) $(( tanh(0) ))"###);
    }

    /// ceil/floor/fabs.
    #[test]
    fn ceil_floor_fabs() {
        assert_parity(r###"zmodload zsh/mathfunc; echo $(( ceil(2.3) )) $(( floor(2.7) )) $(( fabs(-3.5) ))"###);
    }
...
mod zmod_datetime {
    ...
    #[test]
    fn strftime_basic() {
        assert_parity(r###"export TZ=UTC; zmodload zsh/datetime; strftime "%Y-%m-%d %H:%M:%S" 1000000000"###);
    }

    /// strftime specifier set.
    #[test]
    fn strftime_specifiers() {
        assert_parity(r###"export TZ=UTC; zmodload zsh/datetime; strftime "%Y %m %d %H %M %S" 1700000000"###);
    }
    ...
    #[test]
    fn strftime_names() {
        assert_parity(r###"export TZ=UTC; zmodload zsh/datetime; strftime "%A %a %B %b" 1000000000"###);
    }

    /// %p %u %w %s %F %T %C.
    #[test]
    fn strftime_more() {
        assert_parity(r###"export TZ=UTC; zmodload zsh/datetime; strftime "%p|%u|%w|%s|%F|%T|%C" 1700000000"###);
    }

    /// strftime -s assign to scalar.
    #[test]
    fn strftime_s_assign() {
        assert_parity(r###"export TZ=UTC; zmodload zsh/datetime; strftime -s var "%Y-%m-%d" 1700000000; echo $var"###);
    }

    /// strftime -n suppress newline.
    #[test]
    fn strftime_n_nonewline() {
        assert_parity(r###"export TZ=UTC; zmodload zsh/datetime; strftime -n "%Y" 1700000000; echo X"###);
    }

    /// strftime -r reverse parse to epoch.
    #[test]
    fn strftime_r_parse() {
        assert_parity(r###"export TZ=UTC; zmodload zsh/datetime; strftime -r "%Y-%m-%d %H:%M:%S" "2023-11-14 22:13:20""###);
    }

    /// strftime -r -s round-trip.
    #[test]
    fn strftime_r_s() {
        assert_parity(r###"export TZ=UTC; zmodload zsh/datetime; strftime -r -s e "%Y-%m-%d" "2001-09-09"; echo $e"###);
    }
    ...
    fn epochseconds_readonly() {
        assert_parity(r###"zmodload zsh/datetime; [[ $EPOCHSECONDS == <-> ]] && echo intlike; EPOCHSECONDS=5 2>&1; echo "exit=$?""###);
    }
}
...
mod zmod_stat {
    ...
    #[test]
    fn size() {
        assert_parity(r###"zmodload zsh/stat; t=$(mktemp -d); print -n "hello" > $t/f; cd "$t"; zstat +size f; rm -rf "$t""###);
    }

    /// -s symbolic mode.
    #[test]
    fn symbolic_mode() {
        assert_parity(r###"zmodload zsh/stat; t=$(mktemp -d); print -n "x" > $t/f; chmod 644 $t/f; cd "$t"; zstat -s +mode f; rm -rf "$t""###);
    }

    /// -o octal raw mode.
    #[test]
    fn octal_mode() {
        assert_parity(r###"zmodload zsh/stat; t=$(mktemp -d); print -n "x" > $t/f; chmod 644 $t/f; cd "$t"; zstat -o +mode f; rm -rf "$t""###);
    }

    /// +nlink.
    #[test]
    fn nlink() {
        assert_parity(r###"zmodload zsh/stat; t=$(mktemp -d); print -n "x" > $t/f; cd "$t"; zstat +nlink f; rm -rf "$t""###);
    }
    ...
    #[test]
    fn hash_keys() {
        assert_parity(r###"zmodload zsh/stat; t=$(mktemp -d); print -n "hello" > $t/f; cd "$t"; zstat -H h +size f; print -l ${(ok)h}; echo $h[size]; rm -rf "$t""###);
    }

    /// -A array.
    #[test]
    fn array_out() {
        assert_parity(r###"zmodload zsh/stat; t=$(mktemp -d); print -n "hello" > $t/f; cd "$t"; zstat -A arr +size f; echo $arr; rm -rf "$t""###);
    }

    /// -t element-name prefix.
    #[test]
    fn name_prefix() {
        assert_parity(r###"zmodload zsh/stat; t=$(mktemp -d); print -n "hello" > $t/f; cd "$t"; zstat -A arr -t +size f; echo $arr; rm -rf "$t""###);
    }

    /// element abbreviation +si.
    #[test]
    fn abbreviation() {
        assert_parity(r###"zmodload zsh/stat; t=$(mktemp -d); print -n "hello" > $t/f; cd "$t"; zstat +si f; rm -rf "$t""###);
    }

    /// ambiguous abbreviation error.
    #[test]
    fn ambiguous_error() {
        assert_parity(r###"zmodload zsh/stat; t=$(mktemp -d); print -n "x" > $t/f; cd "$t"; zstat +m f 2>&1; echo "exit=$?"; rm -rf "$t""###);
    }

    /// +mtime raw epoch.
    #[test]
    fn mtime_epoch() {
        assert_parity(r###"export TZ=UTC; zmodload zsh/stat; t=$(mktemp -d); print -n "x" > $t/f; touch -t 202001010000.00 $t/f; cd "$t"; zstat +mtime f; rm -rf "$t""###);
    }

    /// -F time format + -g GMT.
    #[test]
    fn time_format_gmt() {
        assert_parity(r###"export TZ=UTC; zmodload zsh/stat; t=$(mktemp -d); print -n "x" > $t/f; touch -t 199501020304.05 $t/f; cd "$t"; zstat -g -F "%Y-%m-%dT%H:%M:%S" +mtime f; rm -rf "$t""###);
    }

    /// -L lstat symlink size.
    #[test]
    fn lstat_symlink() {
        assert_parity(r###"zmodload zsh/stat; t=$(mktemp -d); print -n "x" > $t/f; ln -s f $t/lnk; cd "$t"; zstat -L +size lnk; rm -rf "$t""###);
    }

    /// +link target.
    #[test]
    fn link_target() {
        assert_parity(r###"zmodload zsh/stat; t=$(mktemp -d); print -n "x" > $t/f; ln -s f $t/lnk; cd "$t"; zstat -L +link lnk; rm -rf "$t""###);
    }

    /// -n always show filename.
    #[test]
    fn force_name() {
        assert_parity(r###"zmodload zsh/stat; t=$(mktemp -d); print -n "x" > $t/f; cd "$t"; zstat -n +size f; rm -rf "$t""###);
    }

    /// multiple files names, -N suppresses.
    #[test]
    fn multifile_names() {
        assert_parity(r###"zmodload zsh/stat; t=$(mktemp -d); print -n "ab" > $t/a; print -n "cdef" > $t/b; cd "$t"; zstat +size a b; echo ---; zstat -N +size a b; rm -rf "$t""###);
    }

    /// stat alias name.
    #[test]
    fn stat_alias() {
        assert_parity(r###"zmodload zsh/stat; t=$(mktemp -d); print -n "hi" > $t/f; cd "$t"; stat +size f; rm -rf "$t""###);
    }
...
mod zmod_system {
    ...
    fn flock_acquire_release() {
        assert_parity(r###"zmodload zsh/system; t=$(mktemp -d); f=$t/lk; : >$f; (zsystem flock -f myfd $f; print locked $?; zsystem flock -u $myfd; print unlocked $?); rm -rf $t"###);
    }

    /// zsystem flock subshell auto-release.
    #[test]
    fn flock_subshell() {
        assert_parity(r###"zmodload zsh/system; t=$(mktemp -d); : >$t/f; ( zsystem flock $t/f && print held ); print after=$?; rm -rf $t"###);
    }

    /// zsystem supports known/unknown/syntax.
    #[test]
    fn supports() {
        assert_parity(r###"zmodload zsh/system; zsystem supports flock; print rc=$?; zsystem supports nonsuchthing; print rc=$?; zsystem supports; print rc=$?"###);
    }
    ...
    #[test]
    fn signals() {
        assert_parity(r###"print $signals[1]; print $signals[(i)INT]; print ${signals[(r)TERM]:+T}${signals[(r)ZERR]:+Z}"###);
    }
    ...
    #[test]
    fn sysread_count() {
        assert_parity(r###"zmodload zsh/system; t=$(mktemp -d); print -n "abcde" >$t/f; exec {fd}<$t/f; sysread -c n -i $fd buf; print "$n:$buf"; exec {fd}<&-; rm -rf $t"###);
    }

    /// sysread EOF returns 5.
    #[test]
    fn sysread_eof() {
        assert_parity(r###"zmodload zsh/system; t=$(mktemp -d); : >$t/f; exec {fd}<$t/f; sysread -i $fd buf; print rc=$?; exec {fd}<&-; rm -rf $t"###);
    }
    ...
    #[test]
    fn sysopen() {
        assert_parity(r###"zmodload zsh/system; t=$(mktemp -d); print -n "xyz" >$t/f; sysopen -r -u myfd $t/f; sysread -i $myfd b; print "[$b]"; exec {myfd}<&-; rm -rf $t"###);
    }
}
...
mod zmod_files {
    ...
    #[test]
    fn mkdir_p() {
        assert_parity(r###"zmodload zsh/files; t=$(mktemp -d); zf_mkdir -p $t/a/b/c; [[ -d $t/a/b/c ]] && print made; rm -rf $t"###);
    }

    /// zf_mkdir -m mode.
    #[test]
    fn mkdir_m() {
        assert_parity(r###"zmodload zsh/files; t=$(mktemp -d); zf_mkdir -m 700 $t/d; [[ -x $t/d ]] && print ok; rm -rf $t"###);
    }

    /// zf_ln -s symlink.
    #[test]
    fn ln_s() {
        assert_parity(r###"zmodload zsh/files; t=$(mktemp -d); print hi >$t/orig; zf_ln -s $t/orig $t/link; [[ -L $t/link ]] && print "[$(<$t/link)]"; rm -rf $t"###);
    }

    /// zf_ln hard link.
    #[test]
    fn ln_hard() {
        assert_parity(r###"zmodload zsh/files; t=$(mktemp -d); print z >$t/a; zf_ln $t/a $t/b; print "$(<$t/b)"; rm -rf $t"###);
    }

    /// zf_ln -sf replaces existing.
    #[test]
    fn ln_sf() {
        assert_parity(r###"zmodload zsh/files; t=$(mktemp -d); print one >$t/a; print two >$t/b; zf_ln -sf $t/a $t/b; print "$(<$t/b)"; rm -rf $t"###);
    }

    /// zf_chmod then [[ -x ]].
    #[test]
    fn chmod() {
        assert_parity(r###"zmodload zsh/files; t=$(mktemp -d); : >$t/f; zf_chmod 755 $t/f; [[ -x $t/f ]] && print exec; zf_chmod 644 $t/f; [[ -x $t/f ]] || print noexec; rm -rf $t"###);
    }

    /// zf_mv.
    #[test]
    fn mv() {
        assert_parity(r###"zmodload zsh/files; t=$(mktemp -d); print mvd >$t/a; zf_mv $t/a $t/b; [[ -e $t/a ]] || print "gone:[$(<$t/b)]"; rm -rf $t"###);
    }

    /// zf_rm.
    #[test]
    fn rm() {
        assert_parity(r###"zmodload zsh/files; t=$(mktemp -d); : >$t/f; zf_rm $t/f; [[ -e $t/f ]] && print here || print removed; rm -rf $t"###);
    }

    /// zf_rm -r recursive.
    #[test]
    fn rm_r() {
        assert_parity(r###"zmodload zsh/files; t=$(mktemp -d); mkdir -p $t/d/sub; : >$t/d/sub/f; zf_rm -r $t/d; [[ -e $t/d ]] && print here || print gone; rm -rf $t"###);
    }

    /// zf_rm -f missing file ok.
    #[test]
    fn rm_f_missing() {
        assert_parity(r###"zmodload zsh/files; t=$(mktemp -d); zf_rm -f $t/nope; print rc=$?; rm -rf $t"###);
    }

    /// zf_rmdir empty / non-empty.
    #[test]
    fn rmdir() {
        assert_parity(r###"zmodload zsh/files; t=$(mktemp -d); mkdir $t/e; zf_rmdir $t/e; [[ -d $t/e ]] && print here || print removed; mkdir $t/d; : >$t/d/f; zf_rmdir $t/d 2>/dev/null; print rc=$?; rm -rf $t"###);
    }
    ...
    fn features_only_load() {
        assert_parity(r###"zmodload -m -F zsh/files b:zf_\*; t=$(mktemp -d); zf_mkdir $t/q; [[ -d $t/q ]] && print made; rm -rf $t"###);
    }
...
mod zmod_pcre_regex {
    ...
    #[test]
    fn pcre_match_basic() {
        assert_parity(r###"zmodload zsh/pcre; pcre_compile "[0-9]+"; pcre_match "abc123def"; print "[$MATCH]""###);
    }

    /// capture groups set $match.
    #[test]
    fn pcre_captures() {
        assert_parity(r###"zmodload zsh/pcre; pcre_compile "([a-z]+)-([0-9]+)"; pcre_match "foo-42"; print "$match[1]/$match[2]""###);
    }

    /// named groups.
    #[test]
    fn pcre_named() {
        assert_parity(r###"zmodload zsh/pcre; pcre_compile "(?<word>[a-z]+)"; pcre_match "hello"; print "$match[1]""###);
    }

    /// \d perl class.
    #[test]
    fn pcre_perl_class() {
        assert_parity(r###"zmodload zsh/pcre; pcre_compile "\d{3}"; pcre_match "x456y"; print "[$MATCH]""###);
    }
    ...
    fn pcre_lookahead() {
        assert_parity(r###"zmodload zsh/pcre; pcre_compile "foo(?=bar)"; pcre_match "foobar"; print "rc=$? [$MATCH]"; pcre_match "foobaz"; print rc=$?"###);
    }
    ...
    #[test]
    fn pcre_i() {
        assert_parity(r###"zmodload zsh/pcre; pcre_compile -i "abc"; pcre_match "XABCY"; print "[$MATCH]""###);
    }
    ...
    #[test]
    fn pcre_condition() {
        assert_parity(r###"zmodload zsh/pcre; [[ "abc123" -pcre-match "[0-9]+" ]]; print "rc=$? [$MATCH]""###);
    }

    /// pcre_compile -x extended.
    #[test]
    fn pcre_x() {
        assert_parity(r###"zmodload zsh/pcre; pcre_compile -x "\d+ # the number"; pcre_match "abc42"; print "[$MATCH]""###);
    }
    ...
    #[test]
    fn rematch_pcre() {
        assert_parity(r###"zmodload zsh/pcre; setopt REMATCH_PCRE; [[ "ab12" =~ "\d+" ]]; print "rc=$? [$MATCH]""###);
    }

    /// zsh/regex -regex-match sets MATCH and match.
    #[test]
    fn regex_match_builtin() {
        assert_parity(r###"zmodload zsh/regex; [[ alphabetical -regex-match "^a([^a]+)a([^a]+)a" ]]; print "$MATCH|$match[1]|$match[2]""###);
    }
    ...
    fn regex_no_match_unchanged() {
        assert_parity(r###"zmodload zsh/regex; MATCH=orig; [[ "abc" -regex-match "[0-9]+" ]]; print "rc=$? [$MATCH]""###);
    }

    /// BASH_REMATCH option.
    #[test]
    fn bash_rematch() {
        assert_parity(r###"zmodload zsh/regex; setopt BASH_REMATCH; [[ "x99y" -regex-match "([0-9]+)" ]]; print "[$BASH_REMATCH[1]][$BASH_REMATCH[2]]""###);
    }
}
...
mod zmod_mapfile_langinfo {
    ...
    #[test]
    fn mapfile_read() {
        assert_parity(r###"zmodload zsh/mapfile; t=$(mktemp -d); print -n "line1
line2" >$t/f; print "${mapfile[$t/f]}"; rm -rf $t"###);
    ...
    #[test]
    fn mapfile_split() {
        assert_parity(r###"zmodload zsh/mapfile; t=$(mktemp -d); print -n "a
b
c" >$t/f; a=("${(f@)mapfile[$t/f]}"); print "${#a} ${a[2]}"; rm -rf $t"###);
    ...
    #[test]
    fn langinfo_codeset() {
        assert_parity(r###"zmodload zsh/langinfo; print ${+langinfo[CODESET]}; print ${(t)langinfo}"###);
    }
}
...
mod contrib {
    ...
    #[test]
    fn zmv_dryrun() {
        assert_parity(r###"autoload -U zmv; t=$(mktemp -d); cd $t; : > foo.txt; : > bar.txt; zmv -n "(*).txt" "$1.bak"; cd /; rm -rf $t"###);
    }

    /// zmv (#b) backreference swap.
    #[test]
    fn zmv_backref_swap() {
        assert_parity(r###"autoload -U zmv; t=$(mktemp -d); cd $t; : > a-b.c; : > x-y.c; zmv "(#b)(*)-(*).c" "$2-$1.c"; print -l *(N:t) | sort; cd /; rm -rf $t"###);
    }
    ...
    fn zmv_W() {
        assert_parity(r###"autoload -U zmv; t=$(mktemp -d); cd $t; : > one.txt; : > two.txt; zmv -W "*.txt" "*.bak"; print -l *(N:t) | sort; cd /; rm -rf $t"###);
    }

    /// zmv -C copy mode.
    #[test]
    fn zmv_C_copy() {
        assert_parity(r###"autoload -U zmv; t=$(mktemp -d); cd $t; : > src.txt; zmv -C "(*).txt" "$1.copy"; print -l *(N:t) | sort; cd /; rm -rf $t"###);
    }

    /// zmv -L hardlink mode.
    #[test]
    fn zmv_L_link() {
        assert_parity(r###"autoload -U zmv; t=$(mktemp -d); cd $t; : > orig.txt; zmv -L "(*).txt" "$1.lnk"; print -l *(N:t) | sort; [[ orig.lnk -ef orig.txt ]] && print SAMEINODE; cd /; rm -rf $t"###);
    }

    /// zmv numeric range capture.
    #[test]
    fn zmv_numeric() {
        assert_parity(r###"autoload -U zmv; t=$(mktemp -d); cd $t; : > file2; : > file10; zmv -n "file(<->)" "img$1"; cd /; rm -rf $t"###);
    }
    ...
    fn zmv_match_modifier() {
        assert_parity(r###"autoload -U zmv; t=$(mktemp -d); cd $t; : > abc.log; zmv -n "(#m)*.log" "${MATCH:r}.txt"; cd /; rm -rf $t"###);
    }
    ...
    fn zmv_case_flag() {
        assert_parity(r###"autoload -U zmv; t=$(mktemp -d); cd $t; : > FOO.txt; zmv -n "(#b)(*).txt" "${(L)1}.txt"; cd /; rm -rf $t"###);
    }

    /// zmv -p program.
    #[test]
    fn zmv_p_program() {
        assert_parity(r###"autoload -U zmv; t=$(mktemp -d); cd $t; : > z.txt; zmv -n -p print "(*).txt" "$1.zzz"; cd /; rm -rf $t"###);
    }
    ...
    fn zmv_collision() {
        assert_parity(r###"autoload -U zmv; t=$(mktemp -d); cd $t; : > a.txt; : > b.txt; zmv -n "(?).txt" "x.txt" 2>&1; cd /; rm -rf $t"###);
    }
    ...
    #[test]
    fn colors_color_map() {
        assert_parity(r###"autoload -U colors; colors; print -- ${color[red]} ${color[bold]} ${color[bg-blue]} ${color[31]} ${colour[green]}"###);
    }

    /// colors $fg/$bg escapes.
    #[test]
    fn colors_fg_bg() {
        assert_parity(r###"autoload -U colors; colors; print -- ${fg[red]}${bg[green]} | cat -v"###);
    }

    /// colors $fg_bold / $reset_color / $bold_color.
    #[test]
    fn colors_bold_reset() {
        assert_parity(r###"autoload -U colors; colors; print -- ${fg_bold[cyan]}${reset_color}${bold_color} | cat -v"###);
    }

    /// is-at-least true/false/dash-segment.
    #[test]
    fn is_at_least() {
        assert_parity(r###"autoload -U is-at-least; is-at-least 5.0 5.9.1; print $?; is-at-least 6.0 5.9.1; print $?; is-at-least 3.1.6-15 3.1.6-14; print $?"###);
    }
    ...
    #[test]
    fn zmathfuncdef() {
        assert_parity(r###"autoload -U zmathfuncdef; zmathfuncdef cube "$1*$1*$1"; print $(( cube(3) )); zmathfuncdef inc "$1+${2:-1}"; print $(( inc(5) )) $(( inc(5,10) ))"###);
    }
    ...
    #[test]
    fn add_zsh_hook() {
        assert_parity(r###"autoload -U add-zsh-hook; myfn(){ :; }; add-zsh-hook precmd myfn; print -l $precmd_functions; add-zsh-hook -d precmd myfn; print "count=${#precmd_functions}""###);
    }
}
