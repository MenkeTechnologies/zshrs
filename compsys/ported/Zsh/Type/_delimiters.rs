//! Port of `_delimiters` — emit pattern-modifier delimiter chars.
//!
//! Local shell reference:
//! `/opt/homebrew/share/zsh/functions/_delimiters`.
//!
//! Upstream shell source (9 lines):
//! ```text
//! local expl
//! local -a list
//!
...
pub fn _delimiters(state: &mut MainCompleteState, tag: &str) -> bool {
    ...
    if list.is_empty() {
        ...
        crate::ported::_message::_message(
            &mut state.comp,
            &styles,
    ...
    crate::ported::_wanted::_wanted(state, "delimiters", "delimiter", |s| {
        let mut any = false;
        for d in &list {
