//! Port of `_vars` — complete scalar variable names (excluding
//! arrays), plus arrays with `[` subscript auto-suffix.
//!
//! Local shell reference:
//! `/opt/homebrew/share/zsh/functions/_vars`.
//!
//! Upstream shell source (key lines):
//! ```text
//! 16  _parameters -g '^a*' "$@" && ret=0
//! 19  if compset -S '\[*'; then
//! 20    set - -S "" "$@"
//! 21  else
//! 22    set - -qS"${${QIPREFIX:+[}:-\[}" "$@"
//! 23  fi
//! 24  _parameters -g 'a*' "$@" && ret=0
//! ```
//!
