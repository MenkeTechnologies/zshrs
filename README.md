...
- **Native fish-ported line-editor engines, on by default** (even bare `zshrs -f`): syntax highlighting (lexer-driven command/keyword/quote/redirection/path coloring with command-validity and file-existence checks), history autosuggestions (ghost text, accept with →/End, word-wise accept with forward-word), and prefix/substring up-arrow history search — ports of the fish-shell Rust engines that zsh-syntax-highlighting, zsh-autosuggestions, and zsh-history-substring-search recreate in script. Existing plugin config applies (`ZSH_HIGHLIGHT_STYLES`, `ZSH_AUTOSUGGEST_*`, `HISTORY_SUBSTRING_SEARCH_*`); the native engines stay authoritative even when the script plugins are loaded (their config is honored, their widgets are subsumed); `ZSHRS_NATIVE_ZLE_FX=0` disables them all. Per-keystroke passes are wall-clock budgeted (`ZSHRS_ZLE_{HIGHLIGHT,AUTOSUGGEST}_BUDGET_MS`, default 8) so huge directories or PATHs can never lag typing
- **Native bracket/quote auto-pairing** — port of zsh-autopair (insert-closer, skip-over, pair-delete, space padding; `AUTOPAIR_*` config applies)
