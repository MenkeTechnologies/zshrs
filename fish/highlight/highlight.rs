...
use crate::{
    ...
    parse_util::{
        get_process_first_token_offset, locate_cmdsubst_range, slice_length, MaybeParentheses,
    },
    path::{path_as_implicit_cd, path_get_cdpath, path_get_path, paths_are_same_file},
    terminal::Outputter,
    text_face::{parse_text_face, ResettableStyle, SpecifiedTextFace, TextFace, UnderlineStyle},
    threads::assert_is_background_thread,
    tokenizer::{variable_assignment_equals_pos, PipeOrRedir},
};
use fish_color::Color;
use fish_feature_flags::{feature_test, FeatureFlag};
use fish_wcstringutil::string_prefixes_string;
use fish_widestring::{
    wstr, WExt as _, WString, ASCII_MAX, EXPAND_RESERVED_BASE, EXPAND_RESERVED_END, L,
...
#[cfg(test)]
mod tests {
    use super::{highlight_shell, HighlightColorResolver, HighlightRole, HighlightSpec};
    use crate::env::{EnvMode, EnvSetMode, EnvVar, EnvVarFlags, Environment as _};
    use crate::highlight::parse_text_face_for_highlight;
    use crate::operation_context::{OperationContext, EXPANSION_LIMIT_BACKGROUND};
    use crate::prelude::*;
