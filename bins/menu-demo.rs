...
fn populate_auxiliary_caches(cache: &mut CompsysCache) {
    ...
    if !cache.has_zstyles().unwrap_or(true) {
        let styles = zsh::compsys::parse_zstyles_from_config();
    ...
    loop {
        ...
        if menu.count() > 0 {
            let nav_key = match input {
                ...
                &[0x00] => Some(zsh::compsys::MenuAction::Accept),
                // ^D (0x04) = accept-and-menu-complete - accept + run NEW completion
                &[0x04] => Some(zsh::compsys::MenuAction::AcceptAndMenuComplete),
                // ^F (0x06) = accept-and-infer-next-history - accept + run NEW completion
                &[0x06] => Some(zsh::compsys::MenuAction::AcceptAndInferNextHistory),
                // ^H (0x08) = vi-backward-char - move left
                &[0x08] => Some(zsh::compsys::MenuAction::Left),
                // ^I (0x09) = vi-forward-char (Tab) - move right
                &[0x09] => Some(zsh::compsys::MenuAction::Right),
                // ^J (0x0a) = down-history - move down
                &[0x0a] => Some(zsh::compsys::MenuAction::Down),
                // ^K (0x0b) = up-history - move up
                &[0x0b] => Some(zsh::compsys::MenuAction::Up),
                // ^L (0x0c) = vi-forward-char - move right
                &[0x0c] => Some(zsh::compsys::MenuAction::Right),
                // ^M (0x0d) = .accept-line (Enter) - accept and exit
                &[0x0d] => Some(zsh::compsys::MenuAction::Accept),
                // ^N (0x0e) = vi-forward-word - page down by screenful
                &[0x0e] => Some(zsh::compsys::MenuAction::PageDown),
                // ^P (0x10) = vi-backward-word - page up by screenful
                &[0x10] => Some(zsh::compsys::MenuAction::PageUp),
                // ^S (0x13) = reverse-menu-complete - previous item
                &[0x13] => Some(zsh::compsys::MenuAction::Prev),
                // ^V (0x16) = vi-insert - toggle interactive filter mode
                &[0x16] => Some(zsh::compsys::MenuAction::ToggleInteractive),
                // ^X (0x18) = history-incremental-search-forward
                &[0x18] => Some(zsh::compsys::MenuAction::SearchForward),
                // Ctrl+B (0x02) = backward char (emacs style)
                &[0x02] => Some(zsh::compsys::MenuAction::Left),
                _ => None,
            };
...
fn dump_demo() {
    ...
    for _ in 0..50 {
        scroll_menu.navigate(zsh::compsys::MenuMotion::Down);
    }
    let r = scroll_menu.render();
    if let Some(status) = &r.status {
        println!("Status: {}", status);
    ...
    scroll_menu.navigate(zsh::compsys::MenuMotion::Last);
    let r = scroll_menu.render();
    if let Some(status) = &r.status {
        println!("Status: {}", status);
    }
