...
- ~~**`compinit` async fpath pre-warm** (Phase G2)~~ — **done, but not in `compinit`.** The pre-warm belongs in a process with no live line editor: `parse()` walks process-global lexer state, and compiling on `compinit`'s worker pool beside the interactive main thread corrupted the prompt (xtrace prefix leaking, stuck PS2). The pass now runs as `zshrs --prewarm-autoloads` — at the end of every `zshrs-recorder` run, on demand, or through the daemon as `zd prewarm` — and writes one compiled definition program per completer into `~/.zshrs/autoloads.rkyv`, stamped with the resolved fpath directory, a SHA-256 of the exact definition text and the identity of the compiling binary. The loader (`vm_helper::run_autoload_definition`) reads it on every autoload, so a cold `_git` costs a 229 µs chunk decode instead of 318 ms of parse + compile. See `docs/RECORDER.md` "End-of-run autoload prewarm".
...
- ~~**Bytecode cache for autoloaded functions** (Phase G2)~~ — **done, in rkyv, not SQLite.** `~/.zshrs/autoloads.rkyv` holds one compiled definition program per function (`AutoloadEntry { chunk_blob, source_dir, source_sha, binary_mtime_at_cache, binary_len_at_cache }`, shard format v3), written through on first load and bulk-filled by `--prewarm-autoloads`. Lookup is an O(1) probe into the mmap'd `ArchivedHashMap` — no whole-file scan, no deserialization of other entries. Caching is declined for ksh-style autoload and for `autoload` without `-U`, where the compiled program is not a function of the file's bytes alone.
