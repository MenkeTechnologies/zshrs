...
- ~~**Bytecode cache for autoloaded functions** (Phase G2)~~ — **done, in rkyv, not SQLite.** `~/.zshrs/autoloads.rkyv` holds one compiled definition program per function (`AutoloadEntry { chunk_blob, source_mtime_secs, source_len, binary_mtime_at_cache }`, shard format v2), written through on first load and bulk-filled by `--prewarm-autoloads`. Lookup is an O(1) probe into the mmap'd `ArchivedHashMap` — no whole-file scan, no deserialization of other entries. Caching is declined for ksh-style autoload and for `autoload` without `-U`, where the compiled program is not a function of the file's bytes alone.
