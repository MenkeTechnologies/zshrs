...
- **`cmd | head -0` exit status** — probe artifact, 2026-06-12 sweep B: zshrs's internal `head` applet accepts `-0` GNU-style (prints nothing, rc 0) where macOS BSD `/usr/bin/head` errors `illegal line count -- 0` (rc 1), so the pipeline's `$?` differs. Absorbed-applet semantic, not a zsh-port gap. No real-world script passes `-0`.
