...
pub fn alignmultiwordleft(pos: &mut usize, setpos: i32) -> i32 {       // c:49
    0
}
...
pub fn inccs() {                  // c:122
    ZLECS.fetch_add(1, std::sync::atomic::Ordering::SeqCst);                                                          // c:122
...
/// WARNING: param names don't match C — Rust=(zle) vs C=()
pub fn deccs() {                  // c:133
    ZLECS.fetch_sub(1, std::sync::atomic::Ordering::SeqCst);                                                          // c:133
...
pub fn beginningofline() -> i32 {  // c:298
    ...
    for _ in 0..n {
        ...
        if ZLECS.load(std::sync::atomic::Ordering::SeqCst) > 0 && ZLELINE.lock().unwrap().get(ZLECS.load(std::sync::atomic::Ordering::SeqCst) - 1) == Some(&'\n') {
            ZLECS.fetch_sub(1, std::sync::atomic::Ordering::SeqCst);
            if ZLECS.load(std::sync::atomic::Ordering::SeqCst) == 0 {
                return 0;
            }
        }
        while ZLECS.load(std::sync::atomic::Ordering::SeqCst) > 0 && ZLELINE.lock().unwrap().get(ZLECS.load(std::sync::atomic::Ordering::SeqCst) - 1) != Some(&'\n') {
            ZLECS.fetch_sub(1, std::sync::atomic::Ordering::SeqCst);
        }
    }
    0
...
pub fn endofline() -> i32 {       // c:331
    ...
    for _ in 0..n {
        ...
        while ZLECS.load(std::sync::atomic::Ordering::SeqCst) != ZLELL.load(std::sync::atomic::Ordering::SeqCst) && ZLELINE.lock().unwrap().get(ZLECS.load(std::sync::atomic::Ordering::SeqCst)) != Some(&'\n') {
            ZLECS.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
        }
    }
    0
...
pub fn beginningoflinehist() -> i32 {  // c:360
    ...
    if ZLECS.load(std::sync::atomic::Ordering::SeqCst) == 0 && ZMOD.lock().unwrap().mult > 1 {
...
pub fn forwardchar() -> i32 {     // c:441
    ...
    while ZLECS.load(std::sync::atomic::Ordering::SeqCst) < ZLELL.load(std::sync::atomic::Ordering::SeqCst) && n > 0 {                                   // c:457 while (zlecs < zlell && n--)
        inccs();                                                          // c:458 INCCS()
        n -= 1;
    }
    0                                                                        // c:459 return 0
...
pub fn backwardchar() -> i32 {    // c:464
    ...
    while ZLECS.load(std::sync::atomic::Ordering::SeqCst) > 0 && n > 0 {                                           // c:476 while (zlecs > 0 && n--)
        deccs();                                                          // c:477 DECCS()
        n -= 1;
    }
    0                                                                        // c:478 return 0
...
pub fn setmarkcommand() -> i32 {  // c:483
    if ZMOD.lock().unwrap().mult < 0 {                                                   // c:483 if (zmult < 0)
        REGION_ACTIVE.store(0, std::sync::atomic::Ordering::SeqCst);                                               // c:486
        return 0;                                                            // c:487
    }
    crate::ported::zle::zle_main::MARK.store(ZLECS.load(std::sync::atomic::Ordering::SeqCst), std::sync::atomic::Ordering::SeqCst);                                                    // c:489 mark = zlecs
    REGION_ACTIVE.store(1, std::sync::atomic::Ordering::SeqCst);                                                   // c:490
    0                                                                        // c:491 return 0
}
...
pub fn exchangepointandmark() -> i32 {  // c:496
    if ZMOD.lock().unwrap().mult == 0 {                                                  // c:496 if (zmult == 0)
    ...
    crate::ported::zle::zle_main::MARK.store(ZLECS.load(std::sync::atomic::Ordering::SeqCst), std::sync::atomic::Ordering::SeqCst);                                                    // c:505 mark = zlecs
    ZLECS.store(x, std::sync::atomic::Ordering::SeqCst);                                                           // c:506 zlecs = x
    if ZLECS.load(std::sync::atomic::Ordering::SeqCst) > ZLELL.load(std::sync::atomic::Ordering::SeqCst) {                                               // c:507
        ZLECS.store(ZLELL.load(std::sync::atomic::Ordering::SeqCst), std::sync::atomic::Ordering::SeqCst);                                               // c:508
    }
    if ZMOD.lock().unwrap().mult > 0 {                                                   // c:509
        REGION_ACTIVE.store(1, std::sync::atomic::Ordering::SeqCst);                                               // c:510
    }
    0                                                                        // c:511 return 0
}
...
pub fn visualmode() -> i32 {      // c:516
    ...
    match REGION_ACTIVE.load(std::sync::atomic::Ordering::SeqCst) {                                                // c:524
        1 => { REGION_ACTIVE.store(0, std::sync::atomic::Ordering::SeqCst); }   // c:525-527
        0 => {
            crate::ported::zle::zle_main::MARK.store(ZLECS.load(std::sync::atomic::Ordering::SeqCst), std::sync::atomic::Ordering::SeqCst);                                            // c:529 fall-through to case 2
            REGION_ACTIVE.store(1, std::sync::atomic::Ordering::SeqCst);                                           // c:532
        }
        2 => { REGION_ACTIVE.store(1, std::sync::atomic::Ordering::SeqCst); }   // c:531-533
        _ => {}
    }
    let _ = MOD_CHAR;
    0
...
pub fn visuallinemode() -> i32 {  // c:540
    ...
    //                                  flags |= LINE; return 0 }`.
    match REGION_ACTIVE.load(std::sync::atomic::Ordering::SeqCst) {                                                // c:548
        2 => { REGION_ACTIVE.store(0, std::sync::atomic::Ordering::SeqCst); }   // c:549-551
        0 => {
            crate::ported::zle::zle_main::MARK.store(ZLECS.load(std::sync::atomic::Ordering::SeqCst), std::sync::atomic::Ordering::SeqCst);                                            // c:553
            REGION_ACTIVE.store(2, std::sync::atomic::Ordering::SeqCst);                                           // c:556
        }
        1 => { REGION_ACTIVE.store(2, std::sync::atomic::Ordering::SeqCst); }   // c:555-557
        _ => {}
    }
    let _ = MOD_LINE;
    0
...
pub fn deactivateregion() -> i32 {  // c:564
    REGION_ACTIVE.store(0, std::sync::atomic::Ordering::SeqCst);                                                   // c:564 region_active = 0
    0                                                                        // c:567 return 0
}
...
pub fn vigotocolumn() -> i32 {    // c:572
    ...
    ZLECS.store(target.max(bol).min(eol), std::sync::atomic::Ordering::SeqCst);
    0
}

/// Port of `vimatchbracket(UNUSED(char **args))` from Src/Zle/zle_move.c:594.
pub fn vimatchbracket() -> i32 {  // c:594
    let ocs = ZLECS.load(std::sync::atomic::Ordering::SeqCst);                                                     // c:594
    if (ZLECS.load(std::sync::atomic::Ordering::SeqCst) == ZLELL.load(std::sync::atomic::Ordering::SeqCst) || ZLELINE.lock().unwrap().get(ZLECS.load(std::sync::atomic::Ordering::SeqCst)) == Some(&'\n')) // c:599
        && ZLECS.load(std::sync::atomic::Ordering::SeqCst) > 0
    {
        deccs();                                                          // c:600
    }
    if ZLECS.load(std::sync::atomic::Ordering::SeqCst) == ZLELL.load(std::sync::atomic::Ordering::SeqCst) || ZLELINE.lock().unwrap().get(ZLECS.load(std::sync::atomic::Ordering::SeqCst)) == Some(&'\n') { // c:604
        ZLECS.store(ocs, std::sync::atomic::Ordering::SeqCst);                                                     // c:605
        return 1;                                                            // c:606
    }
    let me = ZLELINE.lock().unwrap()[ZLECS.load(std::sync::atomic::Ordering::SeqCst)];                                         // c:608
    ...
    loop {
        ...
        let c = match ZLELINE.lock().unwrap().get(ZLECS.load(std::sync::atomic::Ordering::SeqCst)) {
            Some(&c) => c,
            None => {
                ZLECS.store(ocs, std::sync::atomic::Ordering::SeqCst);
                return 1;
...
pub fn viforwardchar() -> i32 {   // c:660
    ...
    if ZLECS.load(std::sync::atomic::Ordering::SeqCst) >= lim {                                                    // c:674
        return 1;                                                            // c:675
    }
    while n > 0 && ZLECS.load(std::sync::atomic::Ordering::SeqCst) < lim {                                         // c:676
        inccs();                                                          // c:677
        n -= 1;
    }
    0                                                                        // c:678
...
pub fn vibackwardchar() -> i32 {  // c:683
    ...
    if ZLECS.load(std::sync::atomic::Ordering::SeqCst) == crate::ported::zle::zle_utils::findbol() {            // c:694
        return 1;                                                            // c:695
    }
    while n > 0 && ZLECS.load(std::sync::atomic::Ordering::SeqCst) > 0 {                                           // c:696
        deccs();                                                          // c:697
...
pub fn viendofline() -> i32 {     // c:708
    ...
    for _ in 0..n {
        if ZLECS.load(std::sync::atomic::Ordering::SeqCst) > ZLELL.load(std::sync::atomic::Ordering::SeqCst) {
            ZLECS.store(oldcs, std::sync::atomic::Ordering::SeqCst);
            return 1;
        }
        ZLECS.store(crate::ported::zle::zle_utils::findeol() + 1, std::sync::atomic::Ordering::SeqCst);
    }
    if ZLECS.load(std::sync::atomic::Ordering::SeqCst) > 0 {
        deccs();
    }
...
pub fn vibeginningofline() -> i32 {  // c:728
    ZLECS.store(crate::ported::zle::zle_utils::findbol(), std::sync::atomic::Ordering::SeqCst);                 // c:708
    0                                                                        // c:731
}
...
pub fn vifindchar(repeat: i32) -> i32 {  // c:787
    ...
    if repeat != 0 && tail != 0 {
        if vdir > 0 {
            if ZLECS.load(std::sync::atomic::Ordering::SeqCst) + 1 < ZLELL.load(std::sync::atomic::Ordering::SeqCst) &&
               (ZLELINE.lock().unwrap()[ZLECS.load(std::sync::atomic::Ordering::SeqCst) + 1] as i32) == vfind {
                inccs();
            }
        } else if ZLECS.load(std::sync::atomic::Ordering::SeqCst) > 0 &&
                  (ZLELINE.lock().unwrap()[ZLECS.load(std::sync::atomic::Ordering::SeqCst) - 1] as i32) == vfind {
            deccs();
        }
    }
    ...
    while nn > 0 {                                                           // c:810
        loop {                                                               // c:811-818 do-while
            if vdir > 0 {
                ...
                if ZLECS.load(std::sync::atomic::Ordering::SeqCst) == 0 { break; }
                deccs();
            }
            if { let __c = ZLELINE.lock().unwrap()[ZLECS.load(std::sync::atomic::Ordering::SeqCst)]; ZLECS.load(std::sync::atomic::Ordering::SeqCst) >= ZLELL.load(std::sync::atomic::Ordering::SeqCst) ||
               (__c as i32) == vfind ||
               __c == '\n' } {
                break;
            }
        ...
        if ZLECS.load(std::sync::atomic::Ordering::SeqCst) >= ZLELL.load(std::sync::atomic::Ordering::SeqCst) || ZLELINE.lock().unwrap()[ZLECS.load(std::sync::atomic::Ordering::SeqCst)] == '\n' {
            ZLECS.store(ocs, std::sync::atomic::Ordering::SeqCst);                                                 // c:820
            return 1;
        }
        nn -= 1;
    }
...
pub fn vifirstnonblank() -> i32 {  // c:862
    ZLECS.store(crate::ported::zle::zle_utils::findbol(), std::sync::atomic::Ordering::SeqCst);                 // c:862
    while ZLECS.load(std::sync::atomic::Ordering::SeqCst) != ZLELL.load(std::sync::atomic::Ordering::SeqCst) {                                           // c:865
        let ch = ZLELINE.lock().unwrap()[ZLECS.load(std::sync::atomic::Ordering::SeqCst)];
...
pub fn visetmark(ch: char) -> i32 { // c:872
    ...
    crate::ported::zle::zle_main::vimarks().lock().unwrap()[idx] = Some((ZLECS.load(std::sync::atomic::Ordering::SeqCst), history().lock().unwrap().cursor as i32));        // c:880
    0
}
...
pub fn vigotomark(ch: char) -> i32 { // c:887
    ...
    if let Some((cs, hist)) = crate::ported::zle::zle_main::vimarks().lock().unwrap()[idx] {                            // c:903
        ZLECS.store(cs.min(ZLELL.load(std::sync::atomic::Ordering::SeqCst)), std::sync::atomic::Ordering::SeqCst);
...
pub fn vigotomarkline(ch: char) -> i32 {                                     // c:929
    ...
    pub fn move_up() -> bool {
        ...
        let mut line_start = ZLECS.load(std::sync::atomic::Ordering::SeqCst);
        ...
        ZLECS.store(line_start - 1, std::sync::atomic::Ordering::SeqCst);

        // Find start of previous line
        let mut prev_start = ZLECS.load(std::sync::atomic::Ordering::SeqCst);
        while prev_start > 0 && ZLELINE.lock().unwrap()[prev_start - 1] != '\n' {
            prev_start -= 1;
        }

        // Move to same column or end of line
        ZLECS.store(prev_start + col.min(ZLECS.load(std::sync::atomic::Ordering::SeqCst) - prev_start), std::sync::atomic::Ordering::SeqCst);
    ...
    pub fn move_down() -> bool {
        ...
        let mut line_end = ZLECS.load(std::sync::atomic::Ordering::SeqCst);
        while line_end < ZLELL.load(std::sync::atomic::Ordering::SeqCst) && ZLELINE.lock().unwrap()[line_end] != '\n' {
            line_end += 1;
        }

        if line_end >= ZLELL.load(std::sync::atomic::Ordering::SeqCst) {
            return false; // Already on last line
        }

        // Move to start of next line
        ZLECS.store(line_end + 1, std::sync::atomic::Ordering::SeqCst);

        // Find end of next line
        let mut next_end = ZLECS.load(std::sync::atomic::Ordering::SeqCst);
        while next_end < ZLELL.load(std::sync::atomic::Ordering::SeqCst) && ZLELINE.lock().unwrap()[next_end] != '\n' {
            next_end += 1;
        }

        // Move to same column or end of line
        ZLECS.store((ZLECS.load(std::sync::atomic::Ordering::SeqCst) + col).min(next_end), std::sync::atomic::Ordering::SeqCst);
    ...
    pub fn count_lines() -> usize {
        ZLELINE.lock().unwrap().iter().filter(|&&c| c == '\n').count() + 1
    }
...
mod region_tests {
    ...
    fn inccs_increments_zlecs() {
        ...
        *ZLELINE.lock().unwrap() = "abc".chars().collect();
        ...
        ZLECS.store(0, std::sync::atomic::Ordering::SeqCst);
        inccs();
        assert_eq!(ZLECS.load(std::sync::atomic::Ordering::SeqCst), 1);
        inccs();
        assert_eq!(ZLECS.load(std::sync::atomic::Ordering::SeqCst), 2);
    }
    ...
    fn backwardchar_moves_zmult_positions() {
        ...
        *ZLELINE.lock().unwrap() = "hello world".chars().collect();
        ...
        let r = backwardchar();
        assert_eq!(r, 0);
        assert_eq!(ZLECS.load(std::sync::atomic::Ordering::SeqCst), 5);
    }
    ...
    fn vibeginningofline_jumps_to_bol() {
        ...
        *ZLELINE.lock().unwrap() = "abc\ndef\nghi".chars().collect();
        ...
        assert_eq!(r, 0);
        assert_eq!(ZLECS.load(std::sync::atomic::Ordering::SeqCst), 8); // after the second '\n'
    }
    ...
    fn vibackwardchar_stops_at_line_start() {
        ...
        *ZLELINE.lock().unwrap() = "abc\ndef".chars().collect();
        ...
        assert_eq!(r, 1);
        assert_eq!(ZLECS.load(std::sync::atomic::Ordering::SeqCst), 4); // unchanged
    }
    ...
    fn vifirstnonblank_skips_leading_spaces() {
        ...
        *ZLELINE.lock().unwrap() = "   hello".chars().collect();
        ...
        assert_eq!(r, 0);
        assert_eq!(ZLECS.load(std::sync::atomic::Ordering::SeqCst), 3); // first non-blank
    }
    ...
    fn vifirstnonblank_respects_findbol() {
        ...
        *ZLELINE.lock().unwrap() = "abc\n   def".chars().collect();
        ...
        assert_eq!(ZLECS.load(std::sync::atomic::Ordering::SeqCst), 7);
    }
