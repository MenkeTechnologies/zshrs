...
impl PrivateScope {
    ...
    pub fn is_private(&self, name: &str) -> bool {
        self.params                                                             // c:181
            .get(name)                                                          // c:181
            .map(|p| p.level == self.level)                                     // c:181
            .unwrap_or(false)                                                   // c:181
    }
...
pub fn bin_private(args: &[&str], scope: &mut PrivateScope) -> (i32, String) {
    if args.is_empty() {
...
mod tests {
    ...
    fn test_builtin_private_no_args() {
        let mut scope = PrivateScope::new();
        scope.enter();
        let (status, _) = bin_private(&[], &mut scope);
        assert_eq!(status, 0);
    }
    ...
    fn test_builtin_private_scalar() {
        let mut scope = PrivateScope::new();
        scope.enter();
        let (status, _) = bin_private(&["foo=bar"], &mut scope);
        assert_eq!(status, 0);
        assert!(scope.get("foo").is_some());
    }
    ...
    fn test_builtin_private_integer() {
        let mut scope = PrivateScope::new();
        scope.enter();
        let (status, _) = bin_private(&["-i", "foo=42"], &mut scope);
        assert_eq!(status, 0);
...
impl crate::ported::exec::ShellExecutor {
    ...
    pub(crate) fn bin_private(&mut self, args: &[String]) -> i32 {
...
/// Module loader entry — port of `setup_()` from Src/Modules/param_private.c:670.
pub fn setup_() -> i32 {
    0
}

/// Module loader entry — port of `features_()` from Src/Modules/param_private.c:694.
pub fn features_() -> i32 {
    0
}

/// Module loader entry — port of `enables_()` from Src/Modules/param_private.c:702.
pub fn enables_() -> i32 {
    0
}

/// Module loader entry — port of `boot_()` from Src/Modules/param_private.c:709.
pub fn boot_() -> i32 {
    0
}

/// Module loader entry — port of `cleanup_()` from Src/Modules/param_private.c:717.
pub fn cleanup_() -> i32 {
    0
}

/// Module loader entry — port of `finish_()` from Src/Modules/param_private.c:734.
pub fn finish_() -> i32 {
    0
}
