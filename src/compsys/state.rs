...
// ── `_compskip` global signal ─────────────────────────────────────────
//
// `_compskip` is a shell-side scalar set by `_normal` (Base/Core/_normal:7):
//
//   (( $+opts[-s] )) || _compskip=
//
// i.e. when `-s` is NOT passed, it is CLEARED to empty. Otherwise
// (with `-s`) it stays at whatever the caller set it to. Other
// completers consult `[[ -z $_compskip ]]` to decide whether to fire.
//
// Rust mirror: process-global Option<String>. None = unset (shell
// param non-existent); Some("") = set-but-empty (cleared).

fn compskip_cell() -> &'static Mutex<Option<String>> {
    static C: OnceLock<Mutex<Option<String>>> = OnceLock::new();
    C.get_or_init(|| Mutex::new(None))
}

/// Set the `_compskip` signal to the given value (Some("X") to set,
/// None to unset). Shell behaviour: `_compskip=` sets it to empty
/// string; `unset _compskip` removes it entirely.
pub fn comp_skip_set(value: Option<String>) {
    if let Ok(mut c) = compskip_cell().lock() {
        *c = value;
    }
}

/// Clear `_compskip` to empty string — mirrors shell `_compskip=` in
/// `_normal`. Distinct from [`comp_skip_unset`] which removes it.
pub fn comp_skip_clear_to_empty() {
    comp_skip_set(Some(String::new()));
}

/// Unset `_compskip` entirely — mirrors shell `unset _compskip`.
pub fn comp_skip_unset() {
    comp_skip_set(None);
}

/// Return the current `_compskip` value. `None` means unset.
pub fn comp_skip() -> Option<String> {
    compskip_cell().lock().ok().and_then(|c| c.clone())
}

// ── `precommands` array global ────────────────────────────────────────
//
// `precommands` is set by `_normal` via `-p <pre>` flag: each invocation
// either appends a precommand to the array (`-p pre`) or resets it
// (`-P` clears it). Used by sudo/doas/su/etc. completers to know the
// current "elevated context" prefix chain.

fn precommands_cell() -> &'static Mutex<Vec<String>> {
    static P: OnceLock<Mutex<Vec<String>>> = OnceLock::new();
    P.get_or_init(|| Mutex::new(Vec::new()))
}

/// Replace the `precommands` array with `value`. Mirrors shell
/// `precommands=(...)` assignment, including reset to `()` via
/// `_normal -P`.
pub fn precommands_set(value: Vec<String>) {
    if let Ok(mut p) = precommands_cell().lock() {
        *p = value;
    }
}

/// Append `entry` to `precommands` — mirrors shell `precommands+=(entry)`
/// in `_normal` sh:9 after stripping the leading `-p` from the flag arg.
pub fn precommands_push(entry: String) {
    if let Ok(mut p) = precommands_cell().lock() {
        p.push(entry);
    }
}

/// Return a clone of the current `precommands` array.
pub fn precommands() -> Vec<String> {
    precommands_cell()
        .lock()
        .map(|p| p.clone())
        .unwrap_or_default()
